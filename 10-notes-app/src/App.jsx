import React, { useState } from "react";


const App = () => {

  const [title, settitle] = useState('')
  const [details, setDetails] = useState('')

  // notes ko  task  mai store krane ke liye
     const [task, setTask] = useState([])



  const submithandler = (e)=>{
     e.preventDefault();
    //  console.log(title)
    //  console.log(details)
   
      const copyTask = [...task];
      copyTask.push({title,details})
      setTask(copyTask)
      // console.log(task)

     settitle('')
     setDetails('')
    
  }

    // ye delete krne ke liye function

    function deleteNote(idx){
       const copyTask = [...task]
       copyTask.splice(idx,1)
       setTask(copyTask)
    }

  return (
    <div className="min-h-screen bg-black text-white p-4 md:p-6 lg:p-10">
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
        <form onSubmit={(e)=>{
             submithandler(e)
        }} 
          className="flex items-start w-full lg:w-1/2 gap-4 flex-col">
           <h1 className="text-2xl md:text-3xl font-bold">Add Notes</h1>

         {/* Pehla input ke liye  */}
            <input     
            type="text"
            placeholder="Enter Notes heading"
            className="border-2 px-2 py-2 rounded outline-none font-medium w-full"
            value={title}
            onChange={(e)=>{
              settitle(e.target.value)
            }}

          />

          {/* Textarea wale input ke liye  */}
          <textarea
           
            placeholder="Enter the details"
            className="border-2 h-20 px-2 py-2 rounded outline-none font-medium w-full"
             value={details}
            onChange={(e)=>{
              setDetails(e.target.value)
            }}
          />

          <button className="bg-white text-black active:scale-95 rounded outline-none font-medium px-5 py-2 w-full">
            Add Note
          </button>
            </form>
            
      <div className="w-full lg:w-1/2 lg:border-l-2 lg:pl-6 lg:ml-0">
       <h1 className="text-2xl md:text-3xl font-bold">Recent Notes</h1>
         
         <div className="flex flex-wrap items-start justify-start gap-4 md:gap-5 mt-5 overflow-auto max-h-96 lg:max-h-full">
           {task.map(function(elem,idx){
               return <div key={idx}  className="flex flex-col justify-between items-start  relative h-40 p-5 md:h-44 w-36 md:w-40 py-5 px-4  rounded-2xl bg-[url('data:image/jpeg;base64,')] bg-cover overflow-hidden text-black flex-shrink-0">
                 <div>
                <h3 className="leading-tight text-xl font-bold">{elem.title}</h3>
                <p className="mt-2 leading-tight font-medium text-gray-500">{elem.details}</p>
                 <button onClick={()=>{
                    deleteNote(idx)
                 }} className=" mt-8 w-full cursor-pointer active:scale-95 bg-red-500 py-1 text-xs rounded font-bold text-white">Delete</button>
                 </div>
               
               </div>

           })}
  
         </div>
      
       </div>
      </div>
    </div>
  );
};

export default App;
